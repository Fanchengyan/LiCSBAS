...
def plot_gacos_info(gacos_infofile, pngfile):
    ...
    ax1.scatter(std_bf, std_af, s=sizec, c=colorc,
                marker=markerc, alpha=alphac, zorder=4)
    ax1.set_xlim(0, xylim1)
    ax1.set_ylim(0, xylim1)
    ax1.plot([0, xylim1], [0, xylim1], linewidth=2,
    ...
    # Plot
    ax2.scatter(std_bf, rate, s=sizec, c=colorc,
                marker=markerc, alpha=alphac, zorder=4)
    ax2.plot([0, xylim1], [0, 0], linewidth=2,
             color='grey', alpha=0.5, zorder=2)
    ax2.grid(zorder=0)
...
def plot_hgt_corr(data_bf, fit_hgt, hgt, title, pngfile):
    ...
    hgt_data_bf = np.stack((np.round(hgt[~bool_nan]), np.round(
        data_bf[~bool_nan], 1))).T  # Round values
    hgt_data_bf = np.unique(hgt_data_bf, axis=0)  # Keep only uniques
    hgt_data_af = np.stack((np.round(hgt[~bool_nan]), np.round(
        data_af[~bool_nan], 1))).T  # Round values
    ...
    ax.scatter(hgt_data_bf[:, 0], hgt_data_bf[:, 1],
               s=sbf, c=cbf, marker=mbf, zorder=zbf, label=lbf)
    ax.scatter(hgt_data_af[:, 0], hgt_data_af[:, 1],
               s=saf, c=caf, marker=maf, zorder=zaf, label=laf)
    ax.plot([hgt0, hgt1], [fit_hgt0, fit_hgt1], linewidth=2,
            color='k', alpha=0.8, zorder=8, label='Correction')
...
def plot_network(ifgdates, bperp, rm_ifgdates, pngfile, plot_bad=True):
    ...
    idlist_all = [dt.datetime.strptime(
        x, '%Y%m%d').toordinal() for x in imdates_all]
    ...
    for i, ifgd in enumerate(ifgdates):
        ...
        plt.plot([idlist_all[ix_m], idlist_all[ix_s]], [bperp[ix_m],
    ...
    if plot_bad:
        for i, ifgd in enumerate(rm_ifgdates):
            ...
            plt.plot([idlist_all[ix_m], idlist_all[ix_s]], [
                     bperp[ix_m], bperp[ix_s]], color='r', alpha=0.6, zorder=6, label=label)
    ...
    for i in range(n_im_all):
        ...
        ax.annotate(imdates_all[i][4:6]+'/'+imdates_all[i][6:],
                    (idlist_all[i], bperp[i]), ha='center', va=va, zorder=8)
    ...
    if len(ixs_inc_gap) != 0:
        ...
        plt.vlines(gap_idlist, 0, 1, transform=ax.get_xaxis_transform(
