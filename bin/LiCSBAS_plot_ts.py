...
def calc_model(dph, imdates_ordinal, xvalues, model):
    ...
    if model == 1:  # Annual+L
        ...
        An = np.concatenate(
            (An, sin[:, np.newaxis], cos[:, np.newaxis]), axis=1)
    if model == 2:  # Quad
    ...
    if model == 3:  # Annual+Q
        ...
        cos = np.cos(2*np.pi*imdates_years)
        A = np.concatenate(
            (A, (imdates_years**2)[:, np.newaxis], sin[:, np.newaxis], cos[:, np.newaxis]), axis=1)
        sin = np.sin(2*np.pi*xvalues_years)
        cos = np.cos(2*np.pi*xvalues_years)
        An = np.concatenate(
            (An, (xvalues_years ** 2)[:, np.newaxis], sin[:, np.newaxis], cos[:, np.newaxis]), axis=1)
...
def parse_ref_index(ref_str):
    if re.match(r'\d+:\d+/\d+:\d+', ref_str):
        ref_str = re.match(r'\d+:\d+/\d+:\d+', ref_str).group()
        results = tools_lib.read_range(refarea, width, length)
        return results
    elif re.match(r'[\d, ]+/[\d, ]+', ref_str):
        ref_str = re.match(r'[\d, ]+/[\d, ]+', ref_str).group()
        lon, lat = ref_str.split('/')
        refx = np.fromstring(lon, sep=',', dtype=np.int16)
        refy = np.fromstring(lat, sep=',', dtype=np.int16)
        print(f'{refx}\n {refy}')
        return refy, refx
    else:
        raise ValueError('\nERROR: Reference area/points should be format of x1:x2/y1:y2'
                         ' for rectangle or x1,x2,...,xn/y1,y2,...,yn for points')
...
if __name__ == "__main__":
    ...
    try:
        try:
            opts, args = getopt.getopt(argv[1:], "hi:d:m:r:p:c:", [
                                       "help", "i2=", "nomask", "dmin=", "dmax=", "vmin=", "vmax=", "auto_crange=", "ylen="])
    ...
    if not refarea:
        refy, refx = cumh5['refarea'][()].tolist()
    else:
        refy, refx = parse_ref_index(refarea)

    # Set initial point
    if not point:
        if isinstance(refx, slice):
            point_x = refx.start
            point_y = refy.start
        else:
            point_x = refx[0]
        ...
        results = tools_lib.read_point(point, width, length)
        if not results:
            print('\nERROR in {}\n'.format(point), file=sys.stderr)
            sys.exit(2)
        else:
            point_x, point_y = results
    ...
    if 'deramp_flag' in list(cumh5.keys()):
        ...
        label1 = '1: s={:.1f}km, t={:.2f}yr ({}d){}'.format(
            filtwidth_km, filtwidth_yr, filtwidth_day, deramp)
    else:
        label1 = '1: No filter'
    ...
    if not mdate:  # Not specified or no mdate found in imdates
        ...
        print('No {} found in dates. Set reference to {}'.format(
            mdate, imdates[0]))
        ix_m = 0
    else:
    ...
    if cumfile2:
        ...
        if 'deramp_flag' in list(cumh52.keys()):
            ...
            label2 = '2: s={:.1f}km, t={:.2f}yr ({}d){}'.format(
                filtwidth_km2, filtwidth_yr2, filtwidth_day2, deramp2)
        else:
            label2 = '2: No filter'
    ...
    mapdict_data = {}
    mapdict_unit = {}
    names = ['mask', 'coh_avg', 'n_unw', 'vstd', 'maxTlen', 'n_gap',
             'stc', 'n_ifg_noloop', 'n_loop_err', 'resid', 'mli', 'hgt']
...
#    for name, file in zip(names, files):
    ...
    imdates_dt = np.array(([dt.datetime.strptime(imd, '%Y%m%d')
                            for imd in imdates]))  # datetime
    ...
    refvalue_lastcum = np.nanmean((cum[...][-1, refy, refx] -
                                   cum_ref[:, :][refy, refx])*mask[refy, refx])
    dmin_auto = np.nanpercentile((cum[...][-1, :, :] - cum_ref)*mask,
                                 100-auto_crange)
    dmax_auto = np.nanpercentile((cum[...][-1, :, :] - cum_ref) * mask,
                                 auto_crange)
    ...
    refvalue_vel = np.nanmean((vel*mask)[refy, refx])
    ...
    if vmin is None and vmax is None:  # auto
        ...
        vlimauto = False
        if vmin is None:
            vmin_auto - refvalue_vel
        if vmax is None:
            vmax_auto - refvalue_vel
    ...
    axt2 = pv.text(
        0.01, 0.99, 'Left-doubleclick:\n Plot time series\nRight-drag:\n Change ref area', fontsize=8, va='top')
    axt = pv.text(0.01, 0.78, 'Ref area:\n X {}\n Y {}\n (start from 0)'.format(
        refx, refy), fontsize=8, va='bottom')

    # Fisrt show
    if isinstance(refx, slice):
        refx1h = refx.start-0.5
        refx2h = refx.stop-0.5  # Shift half for plot
        refy1h = refy.start-0.5
        refy2h = refy.stop-0.5
        rax, = axv.plot([refx1h, refx2h, refx2h, refx1h, refx1h],
                        [refy1h, refy1h, refy2h, refy2h, refy1h], '--r', alpha=0.8)
    else:
        rax, = axv.plot(refx, refy, 'ro', alpha=0.8)

    data = vel*mask-np.nanmean((vel*mask)[refy, refx])
    ...
    def line_select_callback(eclick, erelease):
        global refx, refy, dmin, dmax
        # global cannot change existing values... why?

        refxp, refyp = refx, refy  # Previous

        x1, y1 = eclick.xdata, eclick.ydata
        x2, y2 = erelease.xdata, erelease.ydata

        refx = slice(x1, x2, int(((x1 <= x2) - 0.5) * 2))
        refy = slice(y1, y2, int(((y1 <= y2) - 0.5) * 2))

        if np.all(np.isnan(mask[refy, refx])):  # All nan
            print('Selected ref {}/{} has all nan!! Reselect different ref area.'.format(
                refx, refy))
            refx, refy = refxp, refyp  # Get back
            return  # No change
        ...
        # Change clim
        if climauto:  # auto
            refvalue_lastcum = np.nanmean(
                (cum[-1, refy, refx]-cum_ref[refy, refx])*mask[refy, refx])
            dmin = dmin_auto - refvalue_lastcum
            dmax = dmax_auto - refvalue_lastcum
    ...
    RS = RectangleSelector(axv, line_select_callback, drawtype='box', useblit=True, button=[
                           3], spancoords='pixels', interactive=False)
    ...
    def show_vel(val_ind):
        ...
        if 'vel' in val_ind:  # Velocity
            ...
            data = data-np.nanmean(data[refy, refx])
            if vlimauto:  # auto
            ...
            if val_ind == 'hgt':
                cmin_ind = -cmax_ind/3  # bnecause 1/4 of terrain is blue
            cmap2 = 'viridis_r'
            if val_ind in ['coh_avg', 'n_unw', 'mask', 'maxTlen']:
                cmap2 = 'viridis'
            elif val_ind == 'mli':
                cmap2 = SCM.grayC.reversed()
            elif val_ind == 'hgt':
                cmap2 = 'terrain'
            cax.set_cmap(cmap2)
    ...
    tslider = Slider(axtim, 'yr', imdates_ordinal[0]-3, imdates_ordinal[-1] +
                     3, valinit=imdates_ordinal[ix_m], valfmt='')  # %0.0f
    tslider.ax.bar(imdates_ordinal, np.ones(
        len(imdates_ordinal)), facecolor='black', width=4)
...
#        axv.set_title('Time = %s'%(dstr))
        ...
        newv = newv-np.nanmean(newv[refy, refx])
    ...
    axtref = pts.text(0.83, 0.93, 'Ref area:\n X {}\n Y {}\n (start from 0)\nRef date:\n {}'.format(
        refx, refy, imdates[ix_m]), fontsize=8, va='top')
    ...
    def printcoords(event):
        ...
        ii1h = ii-0.5
        ii2h = ii+1-0.5  # Shift half for plot
        jj1h = jj-0.5
        jj2h = jj+1-0.5
        pax.set_data([jj1h, jj2h, jj2h, jj1h, jj1h],
                     [ii1h, ii1h, ii2h, ii2h, ii1h])
        pax2.set_data(jj, ii)
        pv.canvas.draw()
        ...
        if geocod_flag:
            ...
            axtref.set_text('Lat:{:.5f}\nLon:{:.5f}\n\nRef area:\n X {}\n Y {}\n (start from 0)\nRef date:\n {}\n\n{}'.format(
                lat, lon, refx, refy, imdates[ix_m], noisetxt))
        else:
            axtref.set_text('Ref area:\n X {}\n Y {}\n (start from 0)\nRef date:\n {}'.format(
                refx, refy, imdates[ix_m]))
        ...
        # If not masked
        # cumfile
        vel1p = vel[ii, jj]-np.nanmean((vel*mask)[refy, refx])

        dcum_ref = cum_ref[ii, jj]-np.nanmean(
            cum_ref[refy, refx]*mask[refy, refx])
#        dcum_ref = 0
        print((cum.shape, refy, refx), end='\n')
        if isinstance(refx, slice):
            axis = (1, 2)
        else:
            axis = 1
        dph = cum[:, ii, jj]-np.nanmean(cum[...][:, refy, refx]
                                        * mask[refy, refx], axis=axis) - dcum_ref
        ...
        for model, vis in enumerate(visibilities):
            ...
            lines1[model], = axts.plot(
                xvalues, yvalues, 'b-', visible=vis, alpha=0.6, zorder=3)

        axts.scatter(imdates_dt, dph, label=label1, c='b', alpha=0.6, zorder=5)
        axts.set_title(
            'vel = {:.1f} mm/yr @({}, {})'.format(vel1p, jj, ii), fontsize=10)

        # cumfile2
        if cumfile2:
            vel2p = vel2[ii, jj] - \
                np.nanmean((vel2*mask)[refy, refx])
            dcum2_ref = cum2_ref[ii, jj]-np.nanmean(
                cum2_ref[refy, refx]*mask[refy, refx])
            dphf = cum2[:, ii, jj]-np.nanmean(cum2[...][:, refy, refx]
                                              * mask[refy, refx], axis=axis) - dcum2_ref

            # fit function
            lines2 = [0, 0, 0, 0]
            for model, vis in enumerate(visibilities):
                yvalues = calc_model(dphf, imdates_ordinal, xvalues, model)
                lines2[model], = axts.plot(
                    xvalues, yvalues, 'r-', visible=vis, alpha=0.6, zorder=2)

            axts.scatter(imdates_dt, dphf, c='r',
                         label=label2, alpha=0.6, zorder=4)
            axts.set_title('vel(1) = {:.1f} mm/yr, vel(2) = {:.1f} mm/yr @({}, {})'.format(
                vel1p, vel2p, jj, ii), fontsize=10)
        ...
        if gap:
            ...
            if not np.all(~gap1p):  # Not plot if no gap
                gap_ordinal = (
                    imdates_ordinal[1:][gap1p]+imdates_ordinal[0:-1][gap1p])/2
                axts.vlines(gap_ordinal, 0, 1, transform=axts.get_xaxis_transform(
                ), zorder=1, label=label_gap, alpha=0.6, colors='k')
